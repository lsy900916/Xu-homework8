...
**GET** `/api/agent/memory` - 获取对话历史

**DELETE** `/api/agent/memory` - 清空对话历史

### 5.5 Skill/Tool 列表

**GET** `/api/agent/skills` - 获取已注册 Skill 列表

**GET** `/api/agent/tools` - 获取已注册 Tool 列表
...
**原因**：前端传入的 `options` 参数未正确传递

**解决**：确保请求体包含 `options` 字段，如：
...
1. **阅读文档**：
   - `docs/PRD.md` - 了解产品需求
   - `docs/系统架构文档.md` - 了解系统架构（含 Agent Skill 框架）
   - `docs/API_SPEC.md` - 了解 API 规范
   - `docs/Agent_Skill架构对比分析.md` - 了解架构演进

2. **测试功能**：
   - 入库更多测试数据
   - 尝试不同的查询问题
   - 查看聚类分析报告
   - 测试 Agent API 的 options 参数
